...
class Gzh_wz(object):
    ...
    def get_response(self, detail):
        self.detail_res = requests.get(
            self.detail_url,
            headers=self.headers,
            verify=False).text
        con = ''.join(
            re.findall(
                r'<div class="rich_media_content " id="js_content".*?</div>',
                self.detail_res,
                re.DOTALL))
        ...
        if self.title in self.title_l:
            ...
            with open('标题.txt', 'a', encoding='utf-8')as w:
                w.write('\n')
                w.write(self.title)
        self.tt_dict = {x.split('=')[0]: x.split('=')[1]
                        for x in self.detail_url.split('&')}
        print(self.tt_dict)
        ...
        self.gzh_name = ''.join(detail_ret.xpath(
            '//a[@id="js_name"]/text()')).strip()
        self.author = ' '
        if fro:
            self.is_original = '0'
        else:
